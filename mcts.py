                ...
                if node.parent.environments[0].done:
                    ...
                    if conv:
                        if node.environments[0].done and node.environments[1].done:
                            node.terminal=True
                            if node.rewards[0]>=node.rewards[1]:
    ...
    def chmove(self,deterministic):
        ...
        if len(visitcounts)==1:
            ...
            if deterministic:
                ...
                #more than one max, then p is decisive
                else:
                    probs=[]
                ...
                if self.root.level==1 and ac==4 and self.root.actions[ac].n==1:
