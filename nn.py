...
def train_model(generalpath,n_steps,learning_rate,batch_size,
    ...
    while step<n_steps:
        ...
        if step==n_steps-1:
            torch.save(neuralnet.state_dict(), generalpath+"/last")
            ...
            plt.savefig(generalpath+"/loss.pdf")
            plt.close()
            ...
            plt.xlabel(r'Training Steps')
            plt.xticks([0,50,100],('0', str(int(n_steps/2)), str(n_steps)))
            plt.savefig(generalpath+"/cost.pdf")
            plt.close()
...
def use_model(generalpath,depth,alpha,stdnodes,agentlayers,choice,evalsteps):    
    if choice=='last':
        restorepath=generalpath+"/last"
        
    else:
        restorepath=generalpath+"/checkpoint"+str(choice)
    ...
    '''for stepis in range(evalsteps):
        ...
        case1=env.env([np.array([0,0,0,molar_flowrates[0]]),np.array([0,molar_flowrates[1],molar_flowrates[2],0])],1)

        case1.anbauen(0,3,1)
        case1.anbauen(2,0,None)
        case1.anbauen(3,2,None)
        
        s,r1,d1,c1=case1.anbauen(16,None,None)
        print(r1,d1,c1)
        
        case2=env.env([np.array([0,0,0,molar_flowrates[0]]),np.array([0,molar_flowrates[1],molar_flowrates[2],0])],1)
        
        case2.anbauen(0,3,1)
        case2.anbauen(2,0,None)
        case2.anbauen(3,2,None)
        case2.anbauen(4,0,None)
        case2.anbauen(6,2,None)
        
        s,r2,d2,c2=case2.anbauen(16,None,None)
        print(r2,d2,c2)
        
        case3=env.env([np.array([0,0,0,molar_flowrates[0]]),np.array([0,molar_flowrates[1],molar_flowrates[2],0])],1)
        
        case3.anbauen(0,3,1)
        case3.anbauen(2,0,None)
        case3.anbauen(3,2,None)
        case3.anbauen(4,0,None)
        case3.anbauen(6,2,None)

        egal1,egal2,egal3,c4=case3.anbauen(8,4,2)

        if c4:
            s,r4,d4,egal4=case3.anbauen(16,None,None)
            print(r4,d4,c4)
            
        else:
            r4=-1*float('inf')

        if np.max(tree.root.rewards)>=np.max([r1,r2,r4]):
            av1=av1+1
            av3=av3+((np.max(tree.root.rewards)-np.max([r1,r2,r4]))/np.abs(np.max([r1,r2,r4])))
        
        else:
            av4=av4+((np.max(tree.root.rewards)-np.max([r1,r2,r4]))/np.abs(np.max([r1,r2,r4])))
        
        print(np.max(tree.root.rewards),np.max([r1,r2,r4]),((np.max(tree.root.rewards)-np.max([r1,r2,r4]))/np.abs(np.max([r1,r2,r4]))))
        
        av2=av2+((np.max(tree.root.rewards)-np.max([r1,r2,r4]))/np.abs(np.max([r1,r2,r4])))
        
        print('\n',av1,av2/(stepis+1),av3/(stepis+1),av4/(stepis+1),stepis+1,'\n')
        
    print('final',av1/evalsteps,av2/evalsteps,av3/evalsteps,av4/evalsteps)
    f = open(generalpath+"/erg", "a+")
    f.write(str(av1/evalsteps)+'\n')
    f.write(str(av2/evalsteps)+'\n')
    f.write(str(av3/evalsteps)+'\n')
    f.write(str(av4/evalsteps)+'\n')
    f.close()'''
    
    for stepis2 in range(5):
        for stepis in range(5):
            zuwachs=0.2
            molar_flowrates=np.array([0.15+(stepis*zuwachs),0.15+(stepis2*zuwachs),0.15+(stepis2*zuwachs)])
