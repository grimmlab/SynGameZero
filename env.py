...
#filter feasible actions depending on the state and decisions on previous levels (stored in z1 and z2)
def legal(state,level,z1,z2):
    if level==0:
    ...
    if level==1:
        ...
        for i in range(len(state)):
            ...
            #count closed streams without recycles (these can be the destination of a possible recycle)
        ...
        #recycle can only converge if at least two open streams exist and also possible destinations exist
        if closed_withoutrec>0 and openstreams>1:
            feasible[-1]=1
...
#flowsheet is simulated within a tree structure (to be able to work with recycles)
#this function combines information from this tree and the environment to generate the flowsheet matrix
...
class flowsheet:
    ...
    def newapp(self,nr,app,appspec):
        ...
        if app!=N_Unit-1:
            ...
            #mix
            if app==N_Unit-2:
                self.lines[appspec].app=apparatus(app,nr)
                
                newlines=self.lines[appspec].app.compute_output(self.calcinstream(appspec))
                self.streams.append(stream(newlines[0],len(self.streams),appspec,len(self.lines)-1))
                self.lines[appspec].streamsout.append(len(self.streams)-1)
                self.lines[-1].streamsin.append(len(self.streams)-1)
    ...
    #give information on ceratin line of flowsheet
    def giveinf(self,nr):
